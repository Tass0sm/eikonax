...
- `eikonax/strategies/wavefront/` -- `solve(domain, *, source, tol, ...) -> Model`: a single-source field as a partition of unity over LOCAL travel-time models (pure JAX, 1-D only so far). Each splat is a compact window (the SRM density) whose weight is a quadratic model `c + p.d + 1/2 d^T H d` with `|p|` tied to the speed at its centre; a cone splat sits at the source. `chain.py` grows splats outward, ridge to ridge: steps sized so `|n''| r^3/6 <= tol`, emission times by a Hermite rule (O(r^5)), one-sided window radii. No ray marching. `train.py` refines every parameter (residual + consistency + coverage) but is off by default (`train_steps=0`): it lowers its loss while RAISING the true error on an accurate chain (the residual cannot see level drift). `baselines.py` + `scripts/wavefront_1d.py` compare against exact quadrature, oracle scalar-weight SRMs, the literal ridge-mother sum, and `fsm`: uniform speed is exact with 5 splats; the `slow` patch reaches ~7e-6 with 67 splats vs ~2e-3 for 700 scalar splats.
...
- `eikonax/scenarios.py` -- `SCENARIOS` registry of analytic `speed_fn` factories (`free`, `wall`, `gap`, and the smooth `slow` patch) for `--scenario`, since a callable can't be a CLI arg.
